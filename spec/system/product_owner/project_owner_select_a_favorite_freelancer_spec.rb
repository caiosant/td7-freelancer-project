...
describe 'Project Owner select a freelancer as a favorite' do
  it 'and must be signed in' do
    ...
    freelancer.profile.update!({ full_name: 'Caio Free',
    ...
    Proposal.create!({ application_reason: 'Projeto dos meus sonhos, gostaria muito de trabalhar projeto caio',
                       hour_value: 30,
                       hours: 30,
    ...
    visit profile_path(freelancer.profile)

    expect(page).to_not have_content('Perfil de Caio Free')
    expect(page).to_not have_content('Freelancer Favorito')
    expect(page).to_not have_content("Nome Completo: #{freelancer.profile.full_name}")
    expect(page).to_not have_content("Nome Social: #{freelancer.profile.social_name}")
    expect(page).to_not have_content("Data de Nascimento: #{I18n.l(freelancer.profile.birth_date)}")
    expect(page).to_not have_content("Descrição: #{freelancer.profile.description}")
    expect(page).to_not have_link('Retirar dos Favoritos')
  end
  ...
  it 'and got successfully' do
    ...
    freelancer.profile.update!({ full_name: 'Caio Free',
                                 ...
                                 description: 'Grande profissional da área de T.I. com mais ' \
                                              'de 10 anos de exp atuando em grandes empresas' })
    ...
    Proposal.create!({ application_reason: 'Projeto dos meus sonhos, gostaria muito ' \
                                           'de trabalhar projeto caio',
                       hour_value: 30,
                       hours: 30,
                       deadline_proposal: 10.days.from_now,
                       freelancer: freelancer,
