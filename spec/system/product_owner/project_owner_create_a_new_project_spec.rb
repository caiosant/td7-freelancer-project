...
describe 'project owner create a new project' do
  it 'and got successfully' do
    ...
    Ability.create!({ name: 'Javascript' })
    Ability.create!({ name: 'HTML5' })
    Ability.create!({ name: 'CSS' })
    ...
    fill_in 'Descrição',
            with: 'Este projeto da Handa promete garantir qualidade de vida para muitos connect on Linked in. ' \
                  'A Oestia, grupo que promove a intervenção da Handa no público, promoveu há cerca de três anos.'
    ...
    expect(page).to have_content('Descrição: Este projeto da Handa promete garantir qualidade de vida para ' \
                                 'muitos connect on Linked in. A Oestia, grupo que promove a intervenção da Handa ' \
                                 'no público, promoveu há cerca de três anos.')
  ...
  it 'and does not fill nothing' do
    ...
    expect(page).to have_content('Habilidades precisa ter ao mínimo uma opção selecionada')
  ...
  it 'and select a date before today' do
    ...
    fill_in 'Descrição',
            with: 'Este projeto da Handa promete garantir qualidade de vida para muitos connect on Linked in. ' \
                  'A Oestia, grupo que promove a intervenção da Handa no público, promoveu há cerca de três anos.'
    fill_in 'Valor Máximo por Hora', with: 0
    fill_in 'Data Limite para Propostas', with: -1.day.from_now
    select 'Presencial', from: 'Forma de Trabalho'
    within 'form' do
      click_on 'Cadastar Projeto'
    end
